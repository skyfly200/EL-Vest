...
//For spectrum analyzer chip, these three pins are used.
int spectrumReset = 5;
...
// sound detector audio level channel
byte soundAnalog = 1;

// Spectrum analyzer read values will be kept here.
int Spectrum[7];

// sound detector audio level
int soundLvl;
...
// send control message to EL driver mcu over i2c
void sendCtrl(char * ctrl)
{
  Wire.beginTransmission(1); // transmit to device
  Wire.write(ctrl); // send the control
...
// read sound detector level
void readAudioLvl()
{
  soundLvl = analogRead(soundAnalog);
}

// Addressable LED Code
